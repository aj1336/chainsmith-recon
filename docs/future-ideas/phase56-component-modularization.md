...
| 56.9 | **Check-resolver cleanup.** Delete `infer_suite()`, shrink `check_resolver.py`, drop dead imports. **Delete the custom-check registry path** — `CUSTOM_CHECK_REGISTRY` + `_get_custom_checks()` (C9, §6): `custom/` is now an auto-discovered suite, so the tuple list is dead. (Per-component `__init__` re-exports stay — §12 Q1.) | Low |
...
| 56.11 | **Advisors component type.** Port `app/advisors/*` to folder shape. **Migrate the `scan_analysis_advisor`/`check_proof_advisor` sub-configs into each advisor's `config.yaml`** (C10, §5.3), cross-cutting stays in `ChainsmithConfig`. | Low |
...
| 56.16 | **Phase-17 Wave 5:** startup config validation + end-to-end env/override surfacing (the env layer itself is wired in 56.1). | Low |
