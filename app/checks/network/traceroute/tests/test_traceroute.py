...
from app.checks.network.traceroute.check import CDN_PATTERNS, MAX_TARGETS
...
class TestTracerouteCheckRun:
    ...
    async def test_basic_trace_three_hops(self):
        ...
        with (
            ...
            patch("app.checks.network.traceroute.check.asyncio.sleep", return_value=None),
        ):
...
class TestTracerouteEdgeCases:
    ...
    def test_probe_hop_returns_correct_structure_on_connect_refused(self):
        ...
        with (
            patch("app.checks.network.traceroute.check.socket.socket", return_value=mock_sock),
            patch(
                "app.checks.network.traceroute.check.socket.gethostbyaddr",
    ...
    def test_probe_hop_timeout_returns_none_ip(self):
        ...
        with patch("app.checks.network.traceroute.check.socket.socket", return_value=mock_sock):
