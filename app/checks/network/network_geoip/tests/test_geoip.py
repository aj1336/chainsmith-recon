...
from app.checks.network.network_geoip.check import HOSTING_ASNS, RESIDENTIAL_ASNS
...
class TestGeoIpCheckInit:
    ...
    def test_metadata(self):
        check = GeoIpCheck()
        assert check.name == "network_geoip"
...
class TestGeoIpCheckEarlyExits:
    ...
    @patch("app.checks.network.network_geoip.check._find_db_file", side_effect=_find_db_stub)
    ...
    @patch("app.checks.network.network_geoip.check.HAS_GEOIP2", False)
    ...
    @patch("app.checks.network.network_geoip.check._find_db_file", return_value=None)
...
class TestGeoIpClassification:
    ...
    @patch("app.checks.network.network_geoip.check._find_db_file", side_effect=_find_db_stub)
    @patch("app.checks.network.network_geoip.check.geoip2.database.Reader")
    async def test_hosting_asn_classified_as_hosting(self, MockReader, _mock_find):
        ...
        assert obs.check_name == "network_geoip"
...
class TestGeoIpEdgeCases:
    ...
    @patch("app.checks.network.network_geoip.check._find_db_file", side_effect=_find_db_asn_only)
    @patch("app.checks.network.network_geoip.check.geoip2.database.Reader")
