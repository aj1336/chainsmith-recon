...
class TestSSRFIndicatorCheck:
    def test_init(self):
        ...
        assert check.name == "web_ssrf_indicator"
    ...
    async def test_openapi_url_param_detected(self, service):
        ...
        with patch(
            "app.checks.web.web_ssrf_indicator.check.AsyncHttpClient",
    ...
    async def test_exception_in_http_client(self, service):
        ...
        with patch("app.checks.web.web_ssrf_indicator.check.AsyncHttpClient", return_value=mock):
            result = await check.check_service(service, {})
